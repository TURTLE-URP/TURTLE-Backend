...
export class AdminSuppliesService {
  ...
  async findAll() {
    return this.prisma.internal_supplies.findMany({
      include,
      orderBy: { internal_supply_id: 'asc' },
    });
  }
  ...
  async create(dto: CreateSupplyDto) {
    ...
    const existing = await this.prisma.internal_supplies.findUnique({
      where: { code: data.code },
    });
    if (existing) {
      throw new ConflictException(
        `El código de insumo '${data.code}' ya existe`,
      );
    }
    return this.prisma.internal_supplies.create({
      data: {
        ...
        ...(tags?.length
          ? {
              supply_tagging: {
                create: (await this.resolveSupplyTags(tags)).map((t) => ({
  ...
  async update(id: number, dto: UpdateSupplyDto) {
    ...
    if (data.unitOfMeasurementId !== undefined)
      updateData.unit_of_measurement_id = data.unitOfMeasurementId;
    ...
    if (storageRoomIds !== undefined) {
      await this.prisma.internal_supplies_location.deleteMany({
        where: { internal_supply_id: id },
      });
    ...
    if (tags !== undefined) {
      await this.prisma.supply_tagging.deleteMany({
        where: { internal_supply_id: id },
      });
      if (tags.length) {
    ...
    if (Object.keys(updateData).length) {
      await this.prisma.internal_supplies.update({
        where: { internal_supply_id: id },
        data: updateData,
      });
    }
  ...
  async remove(id: number) {
    await this.findOne(id);
    await this.prisma.internal_supplies.delete({
      where: { internal_supply_id: id },
    });
    return { deleted: true };
  }
  ...
  private async resolveSupplyTags(tags: string[]) {
    ...
    const existingMap = new Map(
      existing.map((t) => [t.name, t.internal_supply_tag_id]),
    );
